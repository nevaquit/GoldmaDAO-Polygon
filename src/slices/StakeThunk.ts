...
import { abi as gmaStaking } from "../abi/HectorStakingv2.json";
...
export const changeApproval = createAsyncThunk(  "stake/changeApproval",
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const gmaContract = new ethers.Contract(addresses[networkID].GMA_ADDRESS as string, ierc20Abi, signer);
    const sgmaContract = new ethers.Contract(addresses[networkID].SGMA_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
    let stakeAllowance = await gmaContract.allowance(address, addresses[networkID].STAKING_HELPER_ADDRESS);
    let unstakeAllowance = await sgmaContract.allowance(address, addresses[networkID].STAKING_ADDRESS);
    ...
    try {
      if (token === "gma") {
        ...
        approveTx = await gmaContract.approve(
          addresses[networkID].STAKING_HELPER_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      } else if (token === "sgma") {
        approveTx = await sgmaContract.approve(
          addresses[networkID].STAKING_ADDRESS,
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      }

      const text = "Approve " + (token === "gma" ? "Staking" : "Unstaking");
      const pendingTxnType = token === "gma" ? "approve_staking" : "approve_unstaking";
...
export const changeStake = createAsyncThunk("stake/changeStake",
  async ({ action, value, provider, address, networkID, callback, isOld }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    staking = new ethers.Contract(addresses[networkID].STAKING_ADDRESS as string, gmaStaking, signer);
    ...
    try {
      if (action === "stake") {
        uaData.type = "stake";
        console.log('debug-1', ethers.utils.parseUnits(value, "gwei"), address);
