...
export const loadAppDetails = createAsyncThunk(
  ...
  `;
    ...
    try {
      ...
      const sGmaMainContract = new ethers.Contract(addresses[networkID].SGMA_ADDRESS as string, sHECv2, provider);
      const gmaContract = new ethers.Contract(addresses[networkID].GMA_ADDRESS as string, ierc20Abi, provider);
      const gmaBalance = await gmaContract.balanceOf(addresses[networkID].STAKING_ADDRESS);

      console.log("debug gma", gmaBalance.toString());
      const stakingTVL = (gmaBalance.toNumber()* marketPrice) / 1000000000;
      const circ = await sGmaMainContract.circulatingSupply();
      const circSupply = circ / 1000000000;
      const total = await gmaContract.totalSupply();
    ...
    document.title = `GMADAO - ${marketPriceString}`;
    if (!provider) {
    ...
    console.log('debug->test', marketCap)
    const currentBlock = await provider.getBlockNumber();
    
    // Calculating staking
    const epoch = await stakingContract.epoch();
    console.log('debug->epoch', epoch.distribute);
...
/**
 * - fetches the GMA price from CoinGecko (via getTokenPrice)
